...
const { importUsers, exportUsers, batchAssignRoles, batchDeleteUsers } = require('../../../utils/batchOperations');
...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (updateData.role) {
      ...
      updateData.role = roleExists._id;
    }
...
exports.batchImportUsers = async (req, res) => {
  try {
    ...
    // 调用批量导入工具
    const result = await importUsers(req.file.path);
...
// 批量导出用户
exports.batchExportUsers = async (req, res) => {
  try {
    // 获取筛选条件
    const filter = {};
    if (req.query.role) {
    ...
    // 调用批量导出工具
    const result = await exportUsers(filter);
...
// 批量分配角色
exports.batchAssignUserRoles = async (req, res) => {
  try {
    ...
    // 调用批量分配角色工具
    const result = await batchAssignRoles(userIds, roleName);
